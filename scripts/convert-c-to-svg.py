...
# Default paste target when no input file is given
c_code_input = """
...
def c_array_to_svg(c_text, output_filename="output.svg"):
    ...
    svg_content = [
        ...
        f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {width} {height}" width="{width * 10}" height="{height * 10}" shape-rendering="crispEdges">',
...
def main():
    parser = argparse.ArgumentParser(description="Convert a Piskel C pixel array to SVG.")
    parser.add_argument("input", nargs="?", help="Path to a .c file containing FRAME_WIDTH/HEIGHT and 0x pixel data")
    parser.add_argument("-o", "--output", default="output.svg", help="Output SVG path (default: output.svg)")
    ...
    c_array_to_svg(c_text, args.output)
